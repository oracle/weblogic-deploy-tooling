...
public class RemoveDatabaseWalletCommand extends RemoveTypeCommandBase {
    ...
    @Option(
        names = {"-name"},
        description = "Name of the database wallet to be removed from the archive file",
        required = true
    )
    private String name;
    ...
    public CommandResponse call() throws Exception {
        ...
        try {
            ...
            int entriesRemoved;
            if (this.force) {
                entriesRemoved = this.archive.removeDatabaseWallet(this.name, true);
            } else {
                entriesRemoved = this.archive.removeDatabaseWallet(this.name);
            }
