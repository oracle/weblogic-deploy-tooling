...
@Command(
    name = "script",
    description = "%nAdd script to the archive file:",
    sortOptions = false
)
public class AddScriptCommand extends AddTypeCommandBase {
    ...
    @Option(
        names = {"-source"},
        paramLabel = "<path>",
        description = "File system path to the script to add",
        required = true
    )
    ...
    @Option(
        names = { "-help" },
        description = "Get help for the archiveHelper add script subcommand",
        usageHelp = true
    )
    ...
    public CommandResponse call() throws Exception {
        ...
        try {
            ...
            String resultName;
            if (this.overwrite) {
                resultName = this.archive.replaceScript(sourceFile.getName(), sourceFile.getPath());
            } else {
                resultName = this.archive.addScript(sourceFile.getPath());
            }
