...
public class RemoveClasspathLibraryCommand extends RemoveTypeCommandBase {
    ...
    private static final String TYPE = "classpath library";

    @Option(
        names = {"-name"},
        description = "Name of the classpath library to be removed from the archive file",
        required = true
    )
    private String name;
    ...
    @Option(
        names = { "-help" },
        description = "Get help for the archiveHelper remove classpathLibrary subcommand",
        usageHelp = true
    )
    ...
    public CommandResponse call() throws Exception {
        ...
        try {
            ...
            int entriesRemoved;
            if (this.force) {
                entriesRemoved = this.archive.removeClasspathLibrary(name, true);
            } else {
                entriesRemoved = this.archive.removeClasspathLibrary(name);
            }
