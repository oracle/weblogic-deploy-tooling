...
@Command(
    name = "script",
    header = "Remove script from the archive file.",
    description = "%nCommand-line options:",
    sortOptions = false
)
public class RemoveScriptCommand extends RemoveTypeCommandBase {
    private static final String CLASS = RemoveScriptCommand.class.getName();
    private static final PlatformLogger LOGGER = WLSDeployLogFactory.getLogger(LOGGER_NAME);
    private static final String TYPE = "script";

    @Option(
        names = {"-name"},
        description = "Name of the script to be removed from the archive file",
        required = true
    )
    private String name;
    ...
    @Option(
        names = { "-help" },
        description = "Get help for the archiveHelper remove script subcommand",
        usageHelp = true
    )
    ...
    public CommandResponse call() throws Exception {
        ...
        try {
            ...
            int entriesRemoved;
            if (this.force) {
                entriesRemoved = this.archive.removeScript(this.name, true);
            } else {
                entriesRemoved = this.archive.removeScript(this.name);
            }
