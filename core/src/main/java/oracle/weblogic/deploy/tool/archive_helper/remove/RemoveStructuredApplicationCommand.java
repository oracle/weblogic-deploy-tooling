...
public class RemoveStructuredApplicationCommand extends RemoveTypeCommandBase {
    ...
    @Option(
        names = {"-name"},
        description = "Name of the structured application to be removed from the archive file",
        required = true
    )
    private String name;
    ...
    public CommandResponse call() throws Exception {
        ...
        try {
            ...
            int entriesRemoved;
            if (this.force) {
                entriesRemoved = this.archive.removeStructuredApplication(name, true);
            } else {
                entriesRemoved = this.archive.removeStructuredApplication(name);
            }
