...
@Command(
    ...
    description = "%nAdd classpath library to the archive file:",
    sortOptions = false
)
public class AddClasspathLibraryCommand extends AddTypeCommandBase {
    ...
    @Option(
        names = {"-source"},
        paramLabel = "<path>",
        description = "File system path to the classpath library to add",
        required = true
    )
    ...
    public CommandResponse call() throws Exception {
        ...
        try {
            ...
            String resultName;
            if (this.overwrite) {
                resultName = this.archive.replaceClasspathLibrary(sourceFile.getName(), sourceFile.getPath());
            } else {
                resultName = this.archive.addClasspathLibrary(sourceFile.getPath());
            }
