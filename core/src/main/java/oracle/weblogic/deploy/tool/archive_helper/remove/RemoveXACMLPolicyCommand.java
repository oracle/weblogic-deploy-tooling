...
@Command(
    name = "xacmlPolicy",
    header = "Remove XACML Authorizer policy definition file from the archive file.",
    description = "%nCommand-line options:"
)
public class RemoveXACMLPolicyCommand extends RemoveTypeCommandBase {
    private static final String CLASS = RemoveXACMLPolicyCommand.class.getName();
    private static final PlatformLogger LOGGER = WLSDeployLogFactory.getLogger(LOGGER_NAME);
    private static final String TYPE = "XACML Authorizer policy definition file";

    @Option(
        names = {"-name"},
        description = "Name of the XACML Authorizer policy definition file to be removed from the archive file",
        required = true
    )
    private String name;
    ...
    public CommandResponse call() throws Exception {
        ...
        try {
            ...
            int entriesRemoved;
            if (this.force) {
                entriesRemoved = this.archive.removeXACMLPolicyFile(this.name, true);
            } else {
                entriesRemoved = this.archive.removeXACMLPolicyFile(this.name);
            }
