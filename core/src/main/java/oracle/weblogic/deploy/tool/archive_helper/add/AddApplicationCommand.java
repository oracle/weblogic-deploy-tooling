...
public class AddApplicationCommand extends AddTypeCommandBase {
    ...
    @Option(
        names = {"-source"},
        paramLabel = "<path>",
        description = "File system path to the application to add",
        required = true
    )
    ...
    public CommandResponse call() throws Exception {
        ...
        try {
            ...
            String resultName;
            if (this.overwrite) {
                resultName = this.archive.replaceApplication(sourceFile.getName(), sourceFile.getPath());
            } else {
                resultName = this.archive.addApplication(sourceFile.getPath());
            }
