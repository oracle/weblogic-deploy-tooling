...
public class RemoveMIMEMappingCommand extends RemoveTypeCommandBase {
    ...
    @Option(
        names = {"-name"},
        description = "Name of the MIME mapping file to be removed from the archive file",
        required = true
    )
    private String name;
    ...
    public CommandResponse call() throws Exception {
        ...
        try {
            ...
            int entriesRemoved;
            if (this.force) {
                entriesRemoved = this.archive.removeMimeMappingFile(this.name, true);
            } else {
                entriesRemoved = this.archive.removeMimeMappingFile(this.name);
            }
