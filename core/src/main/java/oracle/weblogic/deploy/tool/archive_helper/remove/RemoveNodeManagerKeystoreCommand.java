...
public class RemoveNodeManagerKeystoreCommand extends RemoveTypeCommandBase {
    ...
    @Option(
        names = {"-name"},
        description = "Name of the node manager keystore to be removed from the archive file",
        required = true
    )
    private String name;
    ...
    public CommandResponse call() throws Exception {
        ...
        try {
            ...
            int entriesRemoved;
            if (this.force) {
                entriesRemoved = this.archive.removeNodeManagerKeystore(this.name, true);
            } else {
                entriesRemoved = this.archive.removeNodeManagerKeystore(this.name);
            }
