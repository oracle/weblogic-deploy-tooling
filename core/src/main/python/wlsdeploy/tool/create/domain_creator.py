...
from wlsdeploy.util import model as model_assist
...
class DomainCreator(Creator):
    ...
    def __init__(self, model_dictionary, model_context, aliases):
        ...
        if model_assist.get_model_domain_info_key() not in model_dictionary:
            ex = exception_helper.create_create_exception('WLSDPLY-12200', self.__program_name,
                                                          model_assist.get_model_domain_info_key(),
    ...
    def __set_app_dir(self):
        ...
        if APP_DIR in self._domain_info:
            ...
            self.logger.fine('WLSDPLY-12225', model_assist.get_model_domain_info_key(), APP_DIR, app_dir,
                             class_name=self.__class_name, method_name=_method_name)
        else:
            ...
            self.logger.fine('WLSDPLY-12226', model_assist.get_model_domain_info_key(), APP_DIR, app_dir,
                             class_name=self.__class_name, method_name=_method_name)
