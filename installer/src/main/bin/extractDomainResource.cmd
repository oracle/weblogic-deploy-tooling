...
IF "%RETURN_CODE%" == "103" (
  ECHO.
  ECHO extractDomainResource.cmd completed successfully but the domain requires a restart for the changes to take effect ^(exit code = %RETURN_CODE%^)
  GOTO exit_script
)
IF "%RETURN_CODE%" == "102" (
  ECHO.
  ECHO extractDomainResource.cmd completed successfully but the effected servers require a restart ^(exit code = %RETURN_CODE%^)
  GOTO exit_script
)
IF "%RETURN_CODE%" == "101" (
  ECHO.
  ECHO extractDomainResource.cmd was unable to complete due to configuration changes that require a domain restart.  Please restart the domain and re-invoke the extractDomainResource.cmd script with the same arguments ^(exit code = %RETURN_CODE%^)
  GOTO exit_script
)
IF "%RETURN_CODE%" == "100" (
  GOTO usage
...
IF "%RETURN_CODE%" == "98" (
  ECHO.
  ECHO extractDomainResource.cmd failed due to a parameter validation error >&2
  GOTO exit_script
)
IF "%RETURN_CODE%" == "2" (
  ECHO.
  ECHO extractDomainResource.cmd failed ^(exit code = %RETURN_CODE%^)
  GOTO exit_script
)
IF "%RETURN_CODE%" == "1" (
  ECHO.
  ECHO extractDomainResource.cmd completed but with some issues ^(exit code = %RETURN_CODE%^) >&2
  GOTO exit_script
)
IF "%RETURN_CODE%" == "0" (
  ECHO.
  ECHO extractDomainResource.cmd completed successfully ^(exit code = %RETURN_CODE%^)
  GOTO exit_script
)
@rem Unexpected return code so just print the message and exit...
ECHO.
ECHO extractDomainResource.cmd failed ^(exit code = %RETURN_CODE%^) >&2
GOTO exit_script
