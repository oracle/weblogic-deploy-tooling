...
ECHO %WLST% %PY_SCRIPTS_PATH%\encrypt.py %*

"%WLST%" "%PY_SCRIPTS_PATH%\encrypt.py" %*
...
IF "%RETURN_CODE%" == "98" (
  ECHO.
  ECHO encryptModel.cmd failed due to a parameter validation error >&2
  GOTO exit_script
)
IF "%RETURN_CODE%" == "2" (
  ECHO.
  ECHO encryptModel.cmd failed ^(exit code = %RETURN_CODE%^)
  GOTO exit_script
)
IF "%RETURN_CODE%" == "1" (
  ECHO.
  ECHO encryptModel.cmd completed but with some issues ^(exit code = %RETURN_CODE%^) >&2
  GOTO exit_script
)
IF "%RETURN_CODE%" == "0" (
  ECHO.
  ECHO encryptModel.cmd completed successfully ^(exit code = %RETURN_CODE%^)
  GOTO exit_script
)
@rem Unexpected return code so just print the message and exit...
ECHO.
ECHO encryptModel.cmd failed ^(exit code = %RETURN_CODE%^) >&2
GOTO exit_script
