...
__logger = PlatformLogger('test.aliases.verify.offline')
__logger.set_level(Level.FINEST)
CLASS_NAME = 'verify_offline'


def verify_offline_aliases(model_context, generated_offline_dict):
    _method_name = 'verify_offline_dictionary'
    __logger.entering(model_context, class_name=CLASS_NAME, method_name=_method_name)

    __logger.info('Verify offline aliases', class_name=CLASS_NAME, method_name=_method_name)
...
def main(args):
    ...
    offline_model_context = verify_utils.get_model_context('verify_offline', WlstModes.OFFLINE, verify_args_map)

    try:
        generated_offline_dictionary = verify_utils.load_generated_offline_dict(offline_model_context)
        verify_results = verify_offline_aliases(offline_model_context, generated_offline_dictionary)
        verify_results.write_to_logger(__logger)
        verify_results.write_report_file(get_report_file_name(offline_model_context))
        system_exit = verify_results.get_exit_code()
    except (StackOverflowError, Exception, JException), e:
        __logger.severe('Verifying the offline aliases failed', error=e, class_name=CLASS_NAME, method_name=_method_name)
        print(dir(traceback))
        traceback.print_exc()
        system_exit = 1
