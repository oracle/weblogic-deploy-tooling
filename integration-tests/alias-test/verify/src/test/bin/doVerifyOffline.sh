...
#
#     DESCRIPTION
#       This script verifies offline mode of a specific version of WebLogic Server
#       alias definitions.  It compares the offline WebLogic version JSON file
...
WLSDEPLOY_PROGRAM_NAME="alias_test_verify_offline"; export WLSDEPLOY_PROGRAM_NAME
...
expected_file_name="${GENERATED_DIR}/$(getOfflineGeneratedFileName "${WLS_VERSION}")"
...
if [ "${RETURN_CODE}" = "0" ]; then
  echo "doVerifyOffline.sh completed successfully"
  touch "${STATUS_DIR}/doVerifyOffline"
else
  echo "doVerifyOffline.sh failed (exit code = ${RETURN_CODE})"
fi
