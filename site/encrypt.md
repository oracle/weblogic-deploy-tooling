...
**NOTE: The current encryption algorithms require JDK 8 to execute, in order to meet Oracle's security standards.  While it is possible to run WLST with a newer JDK than what was used to install WebLogic Server, WLST on the version of WebLogic Server 10.3.6 may not work properly with JDK 8, out of the box, due to JDK 6 JVM arguments that have been removed in JDK8.  It may be necessary to modify the WLST start scripts to remove JVM arguments that have been removed between JDK 6 and JDK 8.**
