...
class MBeanInfoHelper(object):
    ...
    # def __repr__(self):
    #     lists = list()
    #     if len(self.get_child_mbeans()) > 0:
    #         lists.extend(self.get_child_mbeans().keys())
    #     if len(self.get_attributes()) > 0:
    #         lists.extend(self.get_attributes().keys())
    #     lists.sort()
    ...
    def generate_attribute(self, dictionary):
        ...
        if self.__exists:
            ...
            if self.has_since() and all_utils.SINCE_VERSION not in dictionary:
                dictionary[all_utils.SINCE_VERSION] = self.since_version()
            if self.restart_required() and all_utils.RESTART not in dictionary:
                dictionary[all_utils.RESTART] = Boolean(self.restart_required()).toString()
